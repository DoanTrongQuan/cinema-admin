<template>
  <div class="overflow-y-auto p-4">
      <div class = "flex flex-col py-3">
        ...
        <div class = "grid grid-cols-4 gap-2 py-3 items-center">
          <p class = "col-span-1">Tên sự kiện</p>
          <UInput class = "col-span-3" v-model = "promotion.name"  placeholder="Tên sự kiện" size = "xl"></UInput>
        </div>
        <div class = "grid grid-cols-4 gap-3">
          <p class = "col-span-1">Thời gian</p>
          <div class = "flex gap-3">
            <input placeholder="Ngày bắt đầu" class = "col-span-1" v-model = "promotion.start" type="datetime-local" :min = "currentTime">
            <input placeholder="Ngày kết thúc" class = "col-span-1" v-model = "promotion.end" type="datetime-local" :min = "currentTime">
          </div>
        </div>
        <div class="grid grid-cols-4 gap-2 py-3 items-center">
          <p class = "col-span-1">Giảm giá (%)</p>
          <UInput
          size = "xl"
          placeholder="Giảm giá"
            class ="col-span-2"
            v-model="promotion.percent"
            autocomplete="off"
        ...
        <div
          class="grid grid-cols-4 gap-2 py-3 items-center"
        >
        <p class="col-span-1">Đối tượng giảm giá</p>
         <UInputMenu size = "xl" class="col-span-2" v-model="promotion.rank" :options="ranks" />
        </div>
      ...
      <div class = "py-3 grid grid-cols-3 items-center">
        ...
        <div class="flex justify-end">
          <UButton color = "green" class = "col-span-1 mr-5" @click = "createPromotion">Thêm khuyến mãi</UButton>
        </div>
...
function handleFileChange(event) { 
  ...
  const createPromotion = () => {
    const data = {
      ...
      rank:promotion.value.rank,
      percent:promotion.value.percent,
      quantity:promotion.value.quantity,
      image:image.value,
      content:editor.value.getHTML()
    }
    promotionStore.createPromotion(data)
  }
